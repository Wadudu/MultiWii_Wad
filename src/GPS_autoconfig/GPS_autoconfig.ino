/*
UBLOX Autoconfigurator v1.0beta

This sketch put UBLOX GPS in minimal configuration NMEA (GSA,GGA,RMC)
This allow GPS to be configurated at start for next uses.
Soft like Multiwii , will automaticly upload new configuration at next start (UBLOX with optimum parameters)

Ce programme permet de placer le gps en configuration minimale NMEA (GSA,CGA,RMC)
Ceci configure le GPS en mode autonomatique, et il peut etre reconfiguré au prochain démarrage.
Les logiciels comme Multiwii pourront telecharger la nouvelle configuration lors du démarrage de la carte de vol (paramètres optimum pour UBLOX)
